...
def loadFrameworks():
    ...
    frameworks = glob.glob("data/html/*")
    print(len(frameworks))
    for index, html in tqdm(enumerate(frameworks), total=len(frameworks)):
        html_file=open(html, 'r')
        try:
            text = text_from_html(html_file.read())
            frameworks_dict[frameworks[index].split('/')[-1]] = text
        except:
            print("Error processing", frameworks[index].split('/')[-1])

    frameworks = glob.glob("data/txt/*")
    print(len(frameworks))
    for index, txt in tqdm(enumerate(frameworks), total=len(frameworks)):
